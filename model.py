...
def dncnn(input, is_training=True, output_channels=1):
    ...
    for layers in range(2, num_of_hidden_layers + 1): # number of layers was 16 + 1
        print(layers)
        with tf.variable_scope('block%d' % layers):
            output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv%d' % layers, use_bias=False)
            output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
            output = tf.layers.dropout(output,rate=0.2, training=is_training)
    with tf.variable_scope('block' + str(num_of_hidden_layers + 1)): # was 17
...
class denoiser(object):
    def __init__(self, sess, input_c_dim=1, sigma=25, batch_size=128):
        ...
        # blurred_Y_ = tf.map_fn(blur_image, self.Y_)
        
        # noise
        self.X = blurred_Y_ + (tf.random_normal(shape=tf.shape(blurred_Y_), stddev=self.sigma / 255.0) / 10.0)  # noisy images
        print(self.X.shape)
    ...
    def train(self, data, eval_data, batch_size, ckpt_dir, epoch, lr, sample_dir, eval_every_epoch=1):
        ...
        print("[*] Start training, for %d epochs, with start epoch %d start iter %d : " % (epoch, start_epoch, iter_num))
